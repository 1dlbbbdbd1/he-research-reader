  ...
  return rawRecords.map((rawPayload, index) => {
    ...
    return makeRecord({
      ...
      fallbacks: {
        ...
        accessed: firstValue(rawFields.Y2),
        containerTitle: firstValue(rawFields.T2) || firstValue(rawFields.JO) || firstValue(rawFields.JF),
        publisher: firstValue(rawFields.PB),
        publisherPlace: firstValue(rawFields.CY),
  ...
  return rawRecords.map((rawPayload, index) => {
    ...
    return makeRecord({
      ...
      fallbacks: {
        ...
        accessed: firstValue(rawFields.urldate),
        containerTitle: firstValue(rawFields.journal) || firstValue(rawFields.booktitle),
        publisher: firstValue(rawFields.publisher) || firstValue(rawFields.school) || firstValue(rawFields.institution) || firstValue(rawFields.organization),
        publisherPlace: firstValue(rawFields.address) || firstValue(rawFields.location),
  ...
  return records.map((record, index) => {
    ...
    return {
      ...
      normalized: {
        ...
        accessed: textValue(record?.['access-date']) || undefined,
        containerTitle: textValue(record?.periodical?.['full-title']) || textValue(record?.titles?.['secondary-title']) || undefined,
        publisher: textValue(record?.publisher) || textValue(record?.['work-type']) || undefined,
        publisherPlace: textValue(record?.['pub-location']) || undefined,
...
function makeRecord({ ordinal, csl, rawPayload, rawFields, rawRecordId, rawRecordIdField, attachments, fallbacks }) {
  ...
  return {
    ...
    normalized: {
      ...
      accessed: issuedString(csl.accessed) || fallbacks.accessed || undefined,
      containerTitle: csl['container-title'] || fallbacks.containerTitle || undefined,
      publisher: csl.publisher || fallbacks.publisher || undefined,
      publisherPlace: csl['publisher-place'] || fallbacks.publisherPlace || undefined,
