...
const CONVERSATION_WORKFLOWS = Object.freeze([
  {
    ...
    skillId: 'xiaohe-research-cycle', taskType: 'research', sourceSelection: 'optional', minimumSources: 0, maximumSources: 6,
    requiredTools: ['project.inspect'], optionalTools: ['research.source.read'], permissionRequirements: sharedPermissions,
    modelTitle: '形成课题立项与推进计划', modelRationale: '先列目标、资源、导师要求和未知条件，再按立项、检索、证据矩阵、假设、最小基线、单变量实验、复盘写作给出阶段产物与进入下一阶段的证据。每次只推荐一个最小下一步；已有结果引用来源或历史成果 ID。',
  },
  {
    id: 'mentor-review', name: '导师视角审查', category: '选题与规划', featured: false,
    description: '审查问题价值、证据、替代解释和可行性，指出当前最该解决的问题。',
    prompt: '发来方案、进展或困惑；说明希望导师重点追问什么。', outputHint: '优先问题、反例与可执行修改建议', keywords: ['导师', '指导', '质疑', '评审', '瓶颈'],
    skillId: 'xiaohe-research-cycle', taskType: 'research', sourceSelection: 'optional', minimumSources: 0, maximumSources: 6,
    requiredTools: ['project.inspect'], optionalTools: ['research.source.read'], permissionRequirements: sharedPermissions,
    modelTitle: '从导师视角审查研究进展', modelRationale: '按问题价值、主张与证据、方法与对照、资源可行性、复现条件审查。分清阻断问题与可后补问题，为每个问题引用现有材料，提出反例、修正动作和验收证据。给出最多三个值得向真实导师确认的问题，不代替导师批准，不用虚构分数。',
  },
  {
    id: 'idea-discovery', name: '研究思路与最小验证', category: '选题与规划', featured: false,
    description: '从文献冲突、实验异常和资源约束提出候选思路，并设计淘汰或保留它的试验。',
    prompt: '描述卡住的地方、已尝试的方法和异常现象；可附文献或实验记录。', outputHint: '候选思路、反证条件与最小试验', keywords: ['找思路', '创新', '研究空白', '灵感', '假设筛选'],
    skillId: 'xiaohe-research-cycle', taskType: 'research', sourceSelection: 'optional', minimumSources: 0, maximumSources: 6,
    requiredTools: ['project.inspect'], optionalTools: ['research.source.read'], permissionRequirements: sharedPermissions,
    modelTitle: '筛选研究思路与最小验证', modelRationale: '提出至多三个候选，每项说明来自哪条证据或异常、机制假设、替代解释、已有基线、最小验证、成本条件和淘汰标准。新颖性保持待检索。优先推荐一个适合当前资源的动作，保留未采用方案及理由。',
  },
  {
    id: 'experiment-intake', name: '整理散乱实验记录', category: '实验与方法', featured: false,
    description: '把零散笔记、日志和结果描述整理为可核对草稿，确认后保存到课题实验记录。',
    prompt: '说明这批材料属于什么实验；在材料框粘贴日志、笔记、截图描述，或选择已解析资料。', outputHint: '带原文出处的实验记录草稿', keywords: ['散乱', '日志', '实验记录', '整理记录', '实验收件箱'],
    skillId: 'xiaohe-experiment-intake', taskType: 'research', sourceSelection: 'optional', minimumSources: 0, maximumSources: 6,
    requiredTools: ['project.inspect'], optionalTools: ['research.source.read'], permissionRequirements: sharedPermissions,
    modelTitle: '整理实验记录草稿', modelRationale: '按不同实验和运行拆开时间线、目标、环境版本、参数与变量变化、操作、观察结果、异常、解释、决定和待办。每项标来源，参数冲突并列保留，未知日期保持未知，计划不写成已完成。最后列最少补问项。',
  },
  {
    id: 'literature-search', name: '查找相应的文献', category: '文献与阅读', featured: true,
    description: '按研究问题检索 Crossref 与 arXiv，并整理候选文献、来源级别与后续核验建议。',
    prompt: '请说明研究主题、关键词或需要解决的问题。', outputHint: '候选文献清单与全文核验建议', keywords: ['检索', '论文', 'Crossref', 'DOI'],
    skillId: 'xiaohe-literature-evidence',
    taskType: 'research', sourceSelection: 'none', minimumSources: 0, maximumSources: 0,
    requiredTools: ['project.inspect', 'web.fetch'], permissionRequirements: { domains: ['api.crossref.org', 'export.arxiv.org'], applications: [], commands: [] },
  },
  {
  ...
  },
  {
    ...
    skillId: 'xiaohe-research-design',
  ...
  },
  {
    ...
    skillId: 'xiaohe-statistics-review',
  ...
  },
  {
    ...
    skillId: 'xiaohe-scientific-writing',
...
function buildConversationWorkflowSteps(workflow, objective, project, input = {}) {
  ...
  if (workflow.id === 'literature-search') {
    ...
    steps.push({
      kind: 'tool', toolName: 'web.fetch', title: '检索 arXiv 预印本',
      rationale: '补充 arXiv 的公开预印本元数据，并保留预印本与正式出版记录的来源差异。',
      input: { url: `https://export.arxiv.org/api/query?search_query=all:${query}&start=0&max_results=10`, _conversationWorkflowStep: 'search-arxiv' },
    })
    steps.push({ kind: 'model', title: '整理候选文献', rationale: '合并 Crossref 与 arXiv 候选，按 DOI 或 arXiv ID 去重；记录实际查询和来源级别，明确书目信息缺口，并给出下一步全文核验建议。', input: { role: 'executor', _conversationWorkflowStep: 'summarize-candidates' } })
  } else {
...
function inferConversationWorkflow(objective) {
  ...
  if (/找.{0,4}思路|研究思路|寻找.{0,6}(创新|研究空白)|灵感/.test(value)) return getConversationWorkflow('idea-discovery')
  return undefined
}

module.exports = { CONVERSATION_WORKFLOWS, getConversationWorkflow, buildConversationWorkflowSteps, normalizedSourceIds, inferConversationWorkflow }
