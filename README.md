...
- [x] 2026-08-14 Agent 主界面修正版已通过 246 项完整测试、生产构建和隔离桌面 smoke；覆盖固定工作流真实检索、任务授权、项目记忆、资料选择，以及文本/图片/PDF 预览。桌面验收已实际打开资料选择器与项目记忆抽屉，并通过 1024、1600、2K、4K 布局检查。
- [x] 最终安装版与便携版已独立生成到 `release-1.1.0-agent-chat`，成品 smoke 确认四个固定工作流、资料选择器、项目记忆抽屉、模型选择和项目入口可见可用；旧 `release-*` 目录均保留不覆盖。
- [ ] 发布前仍需在专用测试机完成浏览器真实下载/跨域、长时间 Run 和安装/退出/卸载；标为“基础能力”的科研选项也不会冒充完整成品。

当前能力状态以运行前检查为准：

本地成品：安装版 `XiaoHeResearchAssistant-Setup-1.1.0-x64.exe`（342.21 MiB，SHA-256 `66849717535F1911B239CC33030511F59FD5CE5FF4628215CD75CA36237DC2F7`）；便携版 `XiaoHeResearchAssistant-Portable-1.1.0-x64.exe`（101.76 MiB，SHA-256 `1B4A6D1A5A8BD718BA232307FE2C8AB003460F925B602B7C345F779289A6C9BD`）。
