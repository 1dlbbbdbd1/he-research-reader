...
小何的科研助手是一款面向个人研究者的 Windows 本地优先科研 Agent。主界面是一段持续的对话：你可以直接说明要做什么，也可以复用“查找相应的文献、文献分析总结、实验方法指定总结、实验技能教学”等固定工作流。每个本地仓库或研究文件夹都是一个独立项目，对话、记忆、授权范围、任务过程和成果都按项目保存。

当前本地交付版本：**1.1.0**（尚未发布 Release）

## 对话式科研 Agent（1.1.0 本地候选版）

当前版本已把原有科研工作台升级为项目中心制的对话式 Agent。本轮生成本地安装版与便携版并创建本地版本标签；不推送、不发布 GitHub Release。

- [x] 左侧栏管理当前项目、最近项目、项目内对话、科研工作区和设置。
- [x] 中央主界面使用 Codex 风格对话；输入框提供项目内容入口、四个固定工作流、项目资料选择和模型选择。
...
使用“学术文档规范排版”：在“项目”启用后，到“工作台”选择 `.docx` 原文件、新版保存位置和 JSON 模板；授权 Microsoft Word 后先查看结构检查与修改计划，再确认执行。完成后在“运行”页核对变更报告和页面 QA。原文件不会交给 Word 打开，任何已有目标文件也不会被覆盖。
...
其余能力可先在项目菜单的“管理固定工作流”中加入，再从对话框的“+”菜单选择。需要表格或多项条件时，界面会引导你填写对应内容；路线图和科研图表会先产生可编辑结果，确认后的新版本才会用于后续生成。因果推断首次运行还需确认本机分析工具，不能绕过研究设计直接执行模型。
