...
- [x] 中央主界面使用 Codex 风格对话；输入框提供附件/项目内容入口、固定工作流入口和模型选择。
- [x] 每段对话归属于当前项目；项目记忆、执行过程和成果随项目持久化。
- [x] Agent 只能在本次任务授权范围内读取文件、访问网页、运行程序或操作应用；高风险操作仍需再次确认。
- [x] 项目内容可从界面按需浏览和预览，不常驻占用对话空间，也不会擅自移动或删除原文件。
...
- [x] 本地源码验收：239 项测试、生产构建、源码与独立解压版 Electron smoke；源码环境十项能力均显示可试用，独立包中八项可试用，精读/翻译仅因隔离用户数据未安装 Argos 显示缺少工具，十项合同均已接入。
