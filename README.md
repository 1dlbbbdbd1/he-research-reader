...
当前本地候选版本：**1.1.0**（尚未发布 Release）

## 对话式科研 Agent（1.1.0 本地候选版）

当前版本正在把原有科研工作台升级为项目中心制的对话式 Agent。本轮只在现有未提交修改上增量开发并进行本地提交，不推送、不打标签、不发布 Release。
...
- [x] 修复后安装版与便携版已独立生成到 `release-1.1.0-ui-fix`，成品 smoke 确认工作台可滚到底、空能力控件消失；旧 `release-1.1.0` 保留不覆盖。
