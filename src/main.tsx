...
import { rankFrequentWorkflows } from './workflow-history.mjs'
import {
...
function App() {
  ...
  async function sendAgentMessage(input: { objective: string; acceptance: string[]; taskType: 'research' | 'engineering' | 'document' | 'code' | 'data' | 'desktop'; workflow?: DesktopConversationWorkflow; sourceIds?: string[]; pastedText?: string; capabilityPack?: DesktopCapabilityPack; capabilityInput?: Record<string, unknown>; modelRole?: 'planner' | 'executor' | 'vision' | 'verifier' }) {
    const desktop = window.readerDesktop
    if (!desktop) return
    setWorkbenchBusy(true)
    try {
      ...
      const run = await desktop.createWorkbenchRun({
        ...
        conversationWorkflowInput: input.workflow ? { sourceIds: input.sourceIds || [], pastedText: input.pastedText } : undefined,
  ...
  >
    ...
    <main>
      ...
      {active === 'workbench' && <WorkbenchHome
        dashboard={workbenchDashboard} session={activeAgentSession} settings={aiSettings} modelRoles={modelRoles} workflows={conversationWorkflows} capabilityPacks={capabilityPacks} sources={sources} busy={workbenchBusy} pdfScanBusy={projectPdfScanBusy} error={workbenchError}
      ...
      {active === 'sources' && <SourcesV2
        ...
        onAskPaper={sourceId => { setSelectedSource(sourceId); setAgentOpen(true) }}
        onReader={openSource}
...
function WorkflowLibrary({ workflows, capabilityPacks, onClose, onSelectWorkflow, onSelectCapability }: { workflows: DesktopConversationWorkflow[]; capabilityPacks: DesktopCapabilityPack[]; onClose: () => void; onSelectWorkflow: (workflow: DesktopConversationWorkflow) => void; onSelectCapability: (pack: DesktopCapabilityPack) => void }) {
  ...
  const visibleWorkflows = workflows.filter(workflow => matches([workflow.name, workflow.category, workflow.description, workflow.outputHint, workflow.researchSkill?.title, ...(workflow.keywords || [])]))
  ...
  return <div className="workflow-library-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><section ref={dialogRef} className="workflow-library" role="dialog" aria-modal="true" aria-labelledby="workflow-library-title">
    ...
    <div className="workflow-library-body">
      {visibleWorkflows.length > 0 && <section><header><div><strong>科研 Skills</strong><small>按任务加载可复用方法，输出会进入可编辑成果区</small></div><em>{visibleWorkflows.length}</em></header><div className="workflow-library-grid">{visibleWorkflows.map(workflow => <button type="button" className="workflow-library-card" disabled={!workflow.available} key={workflow.id} onClick={() => onSelectWorkflow(workflow)}><span><i>Skill</i><em>{workflow.category}</em></span><strong>{workflow.name}</strong><p>{workflow.description}</p><footer><small>{workflow.outputHint}</small><ChevronRight size={16}/></footer><mark className="workflow-skill-name">方法：{workflow.researchSkill.title} · v{workflow.researchSkill.version}</mark>{!workflow.available && <mark>{workflow.message || '所需工具暂未就绪'}</mark>}</button>)}</div></section>}
...
function WorkbenchHome({ dashboard, session, settings, modelRoles, workflows, capabilityPacks, sources, busy, pdfScanBusy, error, onSend, onOpenProject, onScanProjectPdfs, onOpenRun }: {
  ...
  pdfScanBusy: boolean
  error: string
  onSend: (input: { objective: string; acceptance: string[]; taskType: 'research' | 'engineering' | 'document' | 'code' | 'data' | 'desktop'; workflow?: DesktopConversationWorkflow; sourceIds?: string[]; pastedText?: string; capabilityPack?: DesktopCapabilityPack; capabilityInput?: Record<string, unknown>; modelRole?: 'planner' | 'executor' | 'vision' | 'verifier' }) => void
  onOpenProject: () => void
  ...
  const modelOptions = (Object.entries(modelRoles ?? {}) as Array<[keyof DesktopAppSettings['modelRoles'], DesktopAppSettings['modelRoles'][keyof DesktopAppSettings['modelRoles']]]>)
    .filter(([role, profile]) => role !== 'embedding' && Boolean(profile.model && profile.hasCredential))
    .filter(([, profile]) => !(settings.hasCredential && profile.providerId === settings.providerId && profile.baseUrl === settings.baseUrl && profile.model === settings.model))
    .filter(([, profile], index, entries) => entries.findIndex(([, candidate]) => candidate.providerId === profile.providerId && candidate.baseUrl === profile.baseUrl && candidate.model === profile.model) === index)
  ...
  const commonWorkflowEntries = useMemo(() => rankFrequentWorkflows(dashboard?.runs ?? [], workflows, capabilityPacks, 4) as Array<{ kind: 'workflow' | 'capability'; id: string; useCount: number; lastUsedAt: string }>, [capabilityPacks, dashboard?.runs, workflows])
  const commonWorkflows = commonWorkflowEntries.flatMap(entry => {
    const item = entry.kind === 'workflow' ? workflows.find(workflow => workflow.id === entry.id) : capabilityPacks.find(pack => pack.id === entry.id)
    return item ? [{ ...entry, item }] : []
  })
  const minimumSources = selectedWorkflow?.minimumSources ?? (selectedWorkflow?.sourceSelection === 'required' ? 1 : 0)
  async function importMaterialFiles(files: File[]) {
    try {
      if (files.some(file => file.size > 240000 || !/\.(txt|log|md|csv|json|tsv)$/i.test(file.name))) throw new Error('请选择 240 KB 以内的文本、日志、Markdown、CSV 或 JSON 文件。')
      if (files.reduce((sum, file) => sum + file.size, 0) > 240000) throw new Error('本次文件合计超过 240 KB，请分批导入。')
      const fragments = await Promise.all(files.map(async file => `\n\n--- 来源文件：${file.name} ---\n${await file.text()}`))
      const combined = pastedText + fragments.join('')
      if (combined.length > 60000) throw new Error('合并后的材料超过 60000 字，请分批整理。')
      setPastedText(combined)
      setMaterialError('')
    } catch (error) { setMaterialError(error instanceof Error ? error.message : '读取文件失败，请重试。') }
  }
  ...
  const chooseWorkflow = (workflow: DesktopConversationWorkflow) => { setSelectedWorkflow(workflow); setSelectedCapability(undefined); setCapabilityInput({}); setSourceIds([]); setLibraryOpen(false); setPlusOpen(false) }
  const chooseCapability = (pack: DesktopCapabilityPack) => { setSelectedCapability(pack); setSelectedWorkflow(undefined); setCapabilityInput(initialCapabilityInput(pack)); setSourceIds([]); setLibraryOpen(false); setPlusOpen(false) }
  const clearSelection = () => { setSelectedWorkflow(undefined); setSelectedCapability(undefined); setCapabilityInput({}); setSourceIds([]); setPastedText('') }
  const submit = () => {
    if (!canSubmit) return
    onSend({ objective: objective.trim(), acceptance: [], taskType: selectedWorkflow?.taskType ?? 'research', workflow: selectedWorkflow, sourceIds, pastedText, capabilityPack: selectedCapability, capabilityInput: selectedCapability ? capabilityInput : undefined, modelRole: modelRole || undefined })
    setObjective('')
  }
  return <section className="workbench-page agent-chat-page">
    <div className="agent-chat-scroll">
      {!session?.turns.length && <section className="agent-chat-welcome"><div className="agent-welcome-mark"><Sparkles/></div><h1>今天想研究什么？</h1><p>直接描述任务，小何会自动匹配科研 Skill；也可以手动选择方法和项目资料。任务会先列出范围、步骤和权限，再生成可编辑结果。</p><div className="workflow-starters">{commonWorkflows.map(entry => <button type="button" disabled={entry.kind === 'workflow' ? !(entry.item as DesktopConversationWorkflow).available : !(entry.item as DesktopCapabilityPack).preflight.ready} key={`${entry.kind}:${entry.id}`} onClick={() => entry.kind === 'workflow' ? chooseWorkflow(entry.item as DesktopConversationWorkflow) : chooseCapability(entry.item as DesktopCapabilityPack)}><strong>{entry.item.name}</strong><small>{entry.item.description}</small>{entry.useCount > 0 && <em>用过 {entry.useCount} 次</em>}<ChevronRight size={16}/></button>)}</div><button type="button" className="open-workflow-library" onClick={() => setLibraryOpen(true)}><Blocks size={16}/>浏览全部 {workflows.length + capabilityPacks.length} 个科研工作流<ChevronRight size={16}/></button></section>}
    ...
    <div className="agent-composer-wrap"><div className="agent-composer">
      <nav className="research-cycle-shortcuts" aria-label="科研阶段快捷入口">{['research-kickoff', 'literature-search', 'multi-paper-comparison', 'idea-discovery', 'experiment-design', 'experiment-intake', 'mentor-review'].map(id => { const workflow = workflows.find(item => item.id === id); return workflow && <button type="button" key={id} disabled={busy || !workflow.available} onClick={() => chooseWorkflow(workflow)}>{({ 'research-kickoff': '立项', 'literature-search': '查文献', 'multi-paper-comparison': '证据对比', 'idea-discovery': '找思路', 'experiment-design': '设计实验', 'experiment-intake': '整理记录', 'mentor-review': '导师审查' } as Record<string, string>)[id]}</button> })}</nav>
      {selectedWorkflow && selectedWorkflow.sourceSelection !== 'none' && <details className="research-pasted-material" open={selectedWorkflow.id === 'experiment-intake' || undefined}><summary>粘贴或导入原始材料{pastedText ? ` · ${pastedText.length} 字` : '（可选）'}</summary><label className="research-material-file">导入文本或日志<input type="file" multiple accept=".txt,.log,.md,.csv,.json,.tsv" onChange={event => { const files = [...(event.target.files || [])]; event.target.value = ''; void importMaterialFiles(files) }}/></label>{materialError && <p role="alert" className="workbench-error">{materialError}</p>}<textarea aria-label="原始科研材料" value={pastedText} maxLength={60000} rows={5} onChange={event => setPastedText(event.target.value)} placeholder="直接粘贴实验日志、零散笔记、数据片段或截图文字描述。原文会随任务保存。"/><small>最多 60000 字，可分批整理。实验草稿确认后保存到“课题与实验”；原始记录随整理稿保留。</small></details>}
      {(selectedWorkflow || selectedCapability) && <div className="selected-workflow"><Sparkles size={14}/><span>{selectedWorkflow?.name || selectedCapability?.name}</span><small>{selectedCapability ? '专项执行' : selectedWorkflow?.researchSkill.title}</small><button type="button" aria-label="取消当前工作流" onClick={clearSelection}><X size={13}/></button></div>}
      ...
      <textarea autoFocus maxLength={12000} value={objective} onChange={event => setObjective(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit() } }} placeholder={selectedWorkflow?.prompt ?? selectedCapability?.exampleTask ?? '向小何说明你的研究任务…'}/>
      <footer><div className="composer-left"><div className="composer-plus"><button type="button" className="composer-icon" aria-label="添加内容或选择工作流" onClick={() => setPlusOpen(open => !open)}><Plus size={18}/></button>{plusOpen && <div className="composer-plus-menu"><button type="button" onClick={() => { setPlusOpen(false); onOpenProject() }}><Files size={16}/><span><strong>项目内容</strong><small>浏览并预览当前仓库文件</small></span></button><button type="button" onClick={() => { setPlusOpen(false); setLibraryOpen(true) }}><Blocks size={16}/><span><strong>科研工作流库</strong><small>{workflows.length} 个快速辅助 · {capabilityPacks.length} 个专项执行</small></span><ChevronRight size={15}/></button><div>常用工作流{commonWorkflowEntries.some(entry => entry.useCount > 0) ? ' · 按使用次数排序' : ''}</div>{commonWorkflows.map(entry => <button type="button" disabled={entry.kind === 'workflow' ? !(entry.item as DesktopConversationWorkflow).available : !(entry.item as DesktopCapabilityPack).preflight.ready} key={`${entry.kind}:${entry.id}`} onClick={() => entry.kind === 'workflow' ? chooseWorkflow(entry.item as DesktopConversationWorkflow) : chooseCapability(entry.item as DesktopCapabilityPack)}><Sparkles size={15}/><span><strong>{entry.item.name}</strong>{entry.useCount > 0 && <small>已使用 {entry.useCount} 次</small>}</span></button>)}</div>}</div><select aria-label="选择模型" value={modelRole} onChange={event => setModelRole(event.target.value as typeof modelRole)}><option value="">{settings.model && settings.hasCredential ? `${settings.model} · 默认` : '请先在设置中配置模型'}</option>{modelOptions.map(([role, profile]) => <option value={role} key={role}>{profile.model} · {profile.providerId}</option>)}</select></div><button type="button" className="composer-send" disabled={!canSubmit} onClick={submit}>{busy ? <RotateCcw className="spin" size={17}/> : <ArrowRight size={18}/>}</button></footer>
...
function WorkbenchRuns({
  ...
  return <section className="workbench-page run-console">
    ...
    <div className="run-stage">{run ? <>
      <header className="run-heading"><div><p className="section-kicker">任务编号 {run.id.slice(0, 8)}</p><h1>{run.objective}</h1><span className={`run-status ${run.status}`}>{workbenchStatusLabels[run.status]}</span>{run.researchSkill && <span className="run-skill">科研 Skill：{run.researchSkill.title} · v{run.researchSkill.version}</span>}</div><div className="run-actions">{run.status === 'running' && <><button onClick={() => onNext(run.id)} disabled={busy}>{busy ? '运行中…' : '开始 / 继续运行'}</button><button onClick={() => onPause(run.id)} disabled={busy}>暂停</button></>}{run.status === 'paused' && <button onClick={() => onResume(run.id)} disabled={busy}>恢复</button>}{!['completed', 'failed', 'cancelled'].includes(run.status) && <button className="danger" onClick={() => onCancel(run.id)} disabled={busy}>取消</button>}</div></header>
      {error && <p className="workbench-error"><AlertTriangle size={16}/>{error}</p>}
    ...
    <div className="research-dashboard-grid">
      <section className="research-record-panel">
        ...
        {records.length ? <div className="research-timeline">{records.slice(0, 30).map(record => <article key={record.id} className={`research-record ${record.status}`}>
          <i/><div className="research-record-date" title={record.tags.includes('实验整理') ? '整理登记时间；实验发生时间见记录正文' : '记录时间'}>{record.tags.includes('实验整理') && <span>登记 </span>}{new Date(record.occurredAt).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' })}</div>
...
function SourcesV2({
  ...
  onAskPaper:(sourceId:string)=>void
...
function PaperLibraryTable({
  ...
  return <section className="paper-library-table" aria-label="论文项目">
    <header>
      <span>论文项目</span><span>阅读状态</span><span>阅读进度</span><span>批注</span><span>研究用途</span><span>操作</span>
    </header>
    {rows.map(({ item, source, annotationCount }) => {
      ...
      return <article className={`paper-library-row ${source ? '' : 'without-source'}`} key={item.id}>
        ...
        <div className="paper-library-actions">
          {source && <button className="paper-ai-button" onClick={() => onAskPaper(source.id)}><Sparkles size={13}/>问 AI</button>}
  ...
  const panel = <section className={`agent-modal agent-modal-v2 research-agent-modal ${embedded ? 'embedded' : ''}`}>
    ...
    <div className="agent-answer">
      ...
      {!turns.length && <div className="agent-starter-questions">
        {currentItemId && <button onClick={() => setQuestion('请从这篇论文中提取：')}>提取指定信息</button>}
